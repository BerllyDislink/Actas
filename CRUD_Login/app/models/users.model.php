...
class UsersModel extends Connection
{
    ...
    public function logintoken($correo, $password)
    {
        $row = $this->obtenerDato($correo);
    ...
    public static function obtenerDatoConfirmacion($codigo)
    {
        try {
            $sql = "SELECT * FROM users_sin_confirmar WHERE codigo_correo = :codigo_correo";
            $stmt = Connection::getConnection()->prepare($sql);
            $stmt->bindParam(":codigo_correo", $codigo);
            $stmt->execute();
            $result = $stmt->fetch();
            return $result;
        } catch (PDOException $e) {
    ...
    public static function eliminarDatoConfirmacion($codigo)
    {
        try {
            $sql = 'DELETE FROM users_sin_confirmar WHERE codigo_correo = :codigo_correo';
            $stmt = Connection::getConnection()->prepare($sql);
            $stmt->bindParam(':codigo_correo', $codigo);
            $stmt->execute();
            return true;
