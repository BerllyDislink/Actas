...
class Users
{
    ...
    public function __construct()
    {
        ...
        $this->jwt = new Jwt(getenv('JWT_SECRET_KEY'));
    ...
    public function register()
    {
        ...
        $this->enviarEmail($data['nombres'], $data['correo'], $data['codigo_correo']);
    ...
    public function loginWithToken()
    {
        ...
        if ($this->userModel->obtenerDato($data['correo'])) {
            $loggedInUser = $this->userModel->logintoken($data['correo'], $data['password']);
            if ($loggedInUser) {
                // Generar token JWT
                $payload = [
                    'users_id' => $loggedInUser->users_id,
    ...
    public function verifyToken()
    {
        ...
        if (isset($headers['Authorization'])) {
            ...
            if ($jwt) {
                try {
                    $payload = $this->jwt->decode($jwt);  // Cambiado de encode a decode
                    ...
                    $user = $this->userModel->obtenerDato($payload->users_id);
                    if ($user) {
                        return $user;
                    } else {
            ...
            echo json_encode(['error' => 'Encabezado de autorización no encontrado']);
            return false;
        }
    }
    public function verifyTokenEndpoint()
    {
        ...
        if ($token) {
            try {
                // Cambio: Decodificar el token para obtener el payload como un array
                ...
                echo json_encode(['valid' => false, 'error' => 'Token inválido: ' . $e->getMessage()]);
                return false;
            }
        } else {
    ...
    public function enviarEmail($nombres, $correo, $codigo_correo)
    {
